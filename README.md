...
Description
Bash
Most common shell (like the "friendliest")
sh
...
It is one of the most popular/commonly used shells in Linux.
...
Purpose
/root
Home directory of the root (admin) user. Root user keeps personal files and important settings here.
/boot
Stores files needed to start the Linux system. Contains the Linux kernel and boot loader files.
/tmp
Stores temporary files created by programs. These files are usually deleted automatically after restarting the system.
/var
...
Make directory
cat
Concatenate (create a file with content)
touch
